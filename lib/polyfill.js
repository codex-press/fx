...
Polyfill.prototype._getStylesheets = function() {
  ...
  // TWF changes. this required for it to work with Codex :/
  let re = RegExp('^https://(localhost|usercontent)');
  links = document.getElementsByTagName( "link" )
  while (link = links[i++]) {
    if (link.rel === "stylesheet" && re.test(link.href)) {
      stylesheets.push({ href: link.href })
    }
  }

  // will want to add preview's inline style here as well
